# 개발 실행 스크립트에서 dev report worker 작업을 수행한다.
ì ìííë¤.
﻿param(
    [switch]$Once,
    [int]$BatchSize = 5,
...
$Host.UI.RawUI.WindowTitle = "CAPS report worker"
...
$resolvedQueueBlockSeconds = if ($QueueBlockSeconds -gt 0) {
    ...
    [double](Get-DotEnvValue -EnvPath $envPath -Key "REPORT_JOB_QUEUE_BLOCK_SECONDS" -DefaultValue "15")
}

$resolvedPollIntervalSeconds = if ($PollIntervalSeconds -gt 0) {
    $PollIntervalSeconds
} else {
    [double](Get-DotEnvValue -EnvPath $envPath -Key "REPORT_JOB_FALLBACK_POLL_SECONDS" -DefaultValue "30")
}
...
$arguments = @(
    '-m',
    'server.app.workers.report.generation_worker',
...
Write-Host ""
Write-Host "CAPS report worker 시작" -ForegroundColor Cyan
Write-Host "  루트:       $root"

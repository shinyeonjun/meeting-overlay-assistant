...
from tests.support.sample_inputs import SESSION_TITLE
...
class TestEventLifecycleApi:
    ...
    def test_bulk_transition_confirms_multiple_action_items(self, client, isolated_database):
        session_id = _create_session(client)
        repository = SQLiteMeetingEventRepository(isolated_database)
        utterance_repository = SQLiteUtteranceRepository(isolated_database)
        ...
        assert {item["state"] for item in payload["items"]} == {"confirmed"}
        assert {item["assignee"] for item in payload["items"]} == {"팀"}
        assert {item["due_date"] for item in payload["items"]} == {"2026-03-20"}

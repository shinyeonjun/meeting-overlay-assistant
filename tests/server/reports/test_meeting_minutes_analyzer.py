...
def test_llm_meeting_minutes_analyzer가_stt를_보내고_정본_섹션을_반환한다() -> None:
        ...
        "sections": [
            {
                ...
                "opinions": [
                    {
                        "text": "로그인과 회의록 조회를 우선 배포하자는 의견이 제시되었다.",
                        "important_phrases": ["로그인과 회의록 조회"],
                    },
                ],
                "review": [
    ...
    section_item_schema = completion_client.response_schema["properties"]["sections"][
        "items"
    ]["properties"]["opinions"]["items"]
    assert "important_phrases" in section_item_schema["required"]
    assert "공식 문서체 문장" in section_item_schema["properties"]["text"]["description"]
    assert "핵심 명사구" in section_item_schema["properties"]["important_phrases"]["description"]
    ...
    assert "문서는 정확성, 명확성, 간결성을 우선한다." in completion_client.system_prompt
    assert "시각 스타일은 코드 렌더러가 처리한다." in completion_client.system_prompt
    ...
    assert "important_phrases에는 항목 text에 실제로 포함된 구절만 정확히 복사한다." in completion_client.system_prompt
    assert "회의내용 각 항목에는 important_phrases 배열을 항상 포함" in completion_client.system_prompt
    assert "고기류 부족" in completion_client.system_prompt
    assert "유튜브 영상과 노래 제작" in completion_client.system_prompt
    ...
    assert document.sections[0].opinions[0].important_phrases == ("로그인과 회의록 조회",)
    ...
    assert document.decisions[0].important_phrases == ()
...
def test_llm_meeting_minutes_analyzer는_json_파싱_실패시_한번_재시도한다() -> None:
    completion_client = _FakeCompletionClient(
        [
            '{"agenda":"회의록 점검","overview":["JSON이 깨진 응답"]',
            {
                "agenda": "회의록 점검",
                "overview": ["JSON 파싱 실패 후 재시도했다."],
                "sections": [],
                "special_notes": [],
                "decisions": [{"text": "회의록 JSON 재시도 처리를 적용한다."}],
                "follow_up": [],
            },
        ]
    )
    analyzer = LLMMeetingMinutesAnalyzer(
        completion_client,
        config=MeetingMinutesAnalyzerConfig(model="test-model", max_json_retries=1),
    )
    ...
    assert "이전 응답은 JSON 파싱에 실패했습니다." in completion_client.prompts[1]
    assert document is not None
    assert document.summary[0] == "JSON 파싱 실패 후 재시도했다."
    assert document.decisions[0].text == "회의록 JSON 재시도 처리를 적용한다."


def test_llm_meeting_minutes_analyzer는_회의내용이_비면_품질_재시도한다() -> None:
    completion_client = _FakeCompletionClient(
        [
            {
                "agenda": "된장찌개 포지셔닝 전략 논의",
                "overview": ["된장찌개를 메인 요리로 포지셔닝하는 방안을 논의했다."],
                "sections": [],
                "special_notes": [],
                "decisions": [
                    {
                        "text": "된장찌개를 메인 요리로 포지셔닝하기로 함.",
                        "important_phrases": ["메인 요리"],
                    }
                ],
                "follow_up": [],
            },
                ...
                "sections": [
                    {
                        ...
                        "background": [
                            {
                                "text": "된장찌개를 단순 국물 요리가 아닌 메인 요리로 인식시키는 방향이 논의됨.",
                                "important_phrases": ["메인 요리"],
                            }
                        ],
                        "opinions": [
                            {
                                "text": "고기류와 다양한 천연 재료를 활용해 깊은 맛을 강화할 필요성이 제기됨.",
                                "important_phrases": ["고기류", "천연 재료"],
                            }
                        ],
                        "review": [],
                        "direction": [
                            {
                                "text": "된장찌개 포지셔닝을 위한 레시피 개발과 홍보 콘텐츠 제작을 진행하기로 정리함.",
                                "important_phrases": ["레시피 개발", "홍보 콘텐츠"],
                            }
                        ],
                    }
                ],
    ...
    document = analyzer.analyze(
        ...
        speaker_transcript=[
            SpeakerTranscriptSegment(
                ...
                start_ms=index * 1000,
                end_ms=(index + 1) * 1000,
                text=f"된장찌개를 메인 요리로 포지셔닝하는 전략 발화 {index}",
                confidence=0.9,
            )
    ...
    assert completion_client.call_count == 2
    assert "회의록 품질 검증에 실패했습니다." in completion_client.prompts[1]
    assert "회의내용 sections는 회의록 본문" in completion_client.prompts[1]
    assert "JSON 파싱에 실패했습니다." not in completion_client.prompts[1]
    assert document is not None
    assert document.sections[0].title == "메인 요리 포지셔닝"
    assert document.sections[0].opinions[0].important_phrases == ("고기류", "천연 재료")
    assert document.discussion[0].text == "된장찌개를 단순 국물 요리가 아닌 메인 요리로 인식시키는 방향이 논의됨."


def test_llm_meeting_minutes_analyzer는_품질_재시도_후에도_회의내용이_비면_보정한다() -> None:
    completion_client = _FakeCompletionClient(
        [
            {
                "agenda": "된장찌개 포지셔닝 전략 논의",
                ...
                "special_notes": [
                    {
                        "text": "본연의 맛을 잃지 않도록 주의가 필요하다.",
                        "important_phrases": ["본연의 맛"],
                    }
                ],
                "decisions": [
                    {
                        "text": "된장찌개 홍보 콘텐츠 제작을 진행하기로 함.",
                        "important_phrases": ["홍보 콘텐츠"],
                    }
                ],
                "follow_up": [],
            },
    ...
    assert completion_client.call_count == 2
    assert document is not None
    assert document.sections[0].title == "된장찌개 포지셔닝 전략 논의"
    assert document.sections[0].background[0].text == "된장찌개를 메인 요리로 포지셔닝하는 방안을 논의했다."
    assert document.sections[0].review[0].text == "본연의 맛을 잃지 않도록 주의가 필요하다."
    assert document.sections[0].direction[0].text == "된장찌개 홍보 콘텐츠 제작을 진행하기로 함."
    assert document.sections[0].direction[0].important_phrases == ("홍보 콘텐츠",)


def test_llm_meeting_minutes_analyzer_accepts_loose_payload_shapes_when_repairing_sections() -> None:
    completion_client = _FakeCompletionClient(
        [
            {
                "agenda": {"text": "된장찌개 상품 개선 전략 논의"},
                "overview": "된장찌개를 메인 요리로 포지셔닝하는 방안을 논의했다.",
                "sections": [],
                "special_notes": {
                    "content": "본연의 맛을 잃지 않도록 주의가 필요하다.",
                    "keywords": ["본연의 맛"],
                },
                "decisions": {
                    "content": "홍보 영상과 노래 콘텐츠 제작을 진행하기로 함.",
                    "highlights": ["홍보 영상", "노래 콘텐츠"],
                },
                "follow_up": {
                    "task": "홍보 영상 콘티를 작성한다.",
                    "owner": None,
    ...
    assert document.sections[0].title == "된장찌개 상품 개선 전략 논의"
    assert document.sections[0].background[0].text == "된장찌개를 메인 요리로 포지셔닝하는 방안을 논의했다."
    assert document.sections[0].review[0].text == "본연의 맛을 잃지 않도록 주의가 필요하다."
    assert document.sections[0].direction[0].text == "홍보 영상과 노래 콘텐츠 제작을 진행하기로 함."
    assert document.sections[0].direction[0].important_phrases == ("홍보 영상", "노래 콘텐츠")
    assert document.action_items[0].task == "홍보 영상 콘티를 작성한다."


def test_llm_meeting_minutes_analyzer는_follow_up만_있어도_회의내용을_보정한다() -> None:
    completion_client = _FakeCompletionClient(
        [
            {
                "agenda": "된장찌개 홍보 실행 계획 논의",
                "overview": [],
                "sections": [],
                "special_notes": [],
                "decisions": [],
    ...
    assert completion_client.call_count == 2
    assert document is not None
    assert document.sections[0].title == "된장찌개 홍보 실행 계획 논의"
    assert document.sections[0].direction[0].text == "된장찌개 홍보 영상 콘티를 작성한다."
    assert document.action_items[0].task == "된장찌개 홍보 영상 콘티를 작성한다."


def test_llm_meeting_minutes_analyzer는_json_trailing_comma를_보정한다() -> None:
    completion_client = _FakeCompletionClient(
        """
        {
          "agenda": "회의록 점검",
          "overview": ["JSON 보정 처리를 확인했다.",],
          "sections": [],
          "special_notes": [],
          "decisions": [
            {
              "text": "회의록 JSON trailing comma를 보정한다.",
              "important_phrases": ["trailing comma"],
            },
          ],
          "follow_up": []
        }
    ...
    document = analyzer.analyze(
        ...
        speaker_transcript=[
            SpeakerTranscriptSegment(
                ...
                start_ms=0,
                end_ms=1000,
                text="회의록 JSON trailing comma를 보정합시다.",
                confidence=0.9,
            )
        ],
        events=[],
    ...
    assert document is not None
    assert document.summary[0] == "JSON 보정 처리를 확인했다."
    assert document.decisions[0].important_phrases == ()
...
def test_llm_meeting_minutes_analyzer는_분할_chunk가_실패해도_성공_chunk를_병합한다() -> None:
    completion_client = _FakeCompletionClient(
        [
            {
                "agenda": "첫 번째 chunk",
                "overview": ["첫 번째 chunk를 분석했다."],
                "sections": [],
                "special_notes": [],
                "decisions": [{"text": "첫 번째 chunk 결정"}],
                "follow_up": [],
            },
            "not-json",
            "not-json",
            {
                "agenda": "마지막 chunk",
                "overview": ["마지막 chunk를 분석했다."],
                "sections": [
                    {
                        "title": "마지막 논의",
                        "time_range": None,
                        "background": ["마지막 구간의 논의 배경을 정리했다."],
                        "opinions": ["마지막 구간에서 추가 의견이 제시되었다."],
                        "review": [],
                        "direction": ["후속 검토 방향을 정리했다."],
                    }
                ],
                "special_notes": [],
                "decisions": [],
    ...
    assert document.sections[0].background[0].text == "첫 번째 chunk를 분석했다."
    assert document.sections[0].direction[0].text == "첫 번째 chunk 결정"
    assert document.sections[1].direction[0].text == "후속 검토 방향을 정리했다."


def test_llm_meeting_minutes_analyzer는_분할_병합_후_회의내용이_비면_보정한다() -> None:
    completion_client = _FakeCompletionClient(
        [
            {
                "agenda": "된장찌개 상품 개선 전략 논의",
            ...
            {
                "agenda": "된장찌개 상품 개선 전략 논의",
                "overview": ["영상과 노래를 활용한 홍보 방향을 논의했다."],
                "sections": [],
                "special_notes": [],
                "decisions": [
                    {
                        "text": "된장찌개 관련 영상과 노래 콘텐츠 제작을 진행하기로 함.",
                        "important_phrases": ["영상", "노래 콘텐츠"],
                    }
                ],
                "follow_up": [],
            },
    ...
    assert document.sections[0].background[0].text == "된장찌개를 메인 요리로 포지셔닝하는 방안을 논의했다."
    assert document.sections[0].review[0].text == "본연의 맛을 잃지 않도록 주의가 필요하다."
    assert document.sections[0].direction[0].text == "된장찌개 관련 영상과 노래 콘텐츠 제작을 진행하기로 함."
    assert document.sections[0].direction[0].important_phrases == ("영상", "노래 콘텐츠")

...
class TestReportApi:
    ...
    def test_마크다운_리포트_파일이_바뀌면_최신_조회는_파일_본문을_반환한다(self, client):
        session_id = _create_session(client)
    ...
    def test_report_id_조회도_파일_본문을_우선_사용한다(self, client):
        session_id = _create_session(client)
    ...
    def test_세션이_종료되면_final_status가_pending_job상태다(self, client):
        session_id = _create_session(client)

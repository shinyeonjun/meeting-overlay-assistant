...
from backend.app.services.analysis.observability import (
...
class LLMAnalysisResponseParser:
    ...
    def parse(self, response_text: str) -> LLMAnalysisResult:
        ...
        for item in candidates_data:
            ...
            if parsed_candidate is None:
                continue
            dedup_key = (parsed_candidate.event_type, self._normalize_title(parsed_candidate.title))
    ...
    def _parse_candidate(self, item: object) -> LLMEventCandidate | None:
        ...
        if priority is None:
            priority = self._spec.default_priority_by_event_type.get(event_type, 0)
        ...
        return LLMEventCandidate(
            ...
            assignee=self._nullable_string(item.get("assignee")),
            due_date=self._nullable_string(item.get("due_date")),
            topic_group=self._nullable_string(item.get("topic_group")),
        )

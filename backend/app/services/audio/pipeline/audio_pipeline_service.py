...
from backend.app.services.audio.filters.audio_content_gate import AudioContentGate
...
from backend.app.services.audio.segmentation.speech_segmenter import AudioSegmenter
from backend.app.services.audio.stt.transcription import (
    SpeechToTextService,
    StreamingSpeechToTextService,
)
from backend.app.services.audio.filters.transcription_guard import TranscriptionGuard
...
class AudioPipelineService:
    ...
    def process_chunk(
        ...
        logger.debug("오디오 청크 처리 시작: session_id=%s chunk_bytes=%d", session_id, len(chunk))
        ...
        if utterances or saved_events:
            logger.info(
                "오디오 청크 처리 완료: session_id=%s utterances=%d events=%d",
                session_id,
                len(utterances),
                len(saved_events),
            )
        else:
            logger.debug("오디오 청크 처리 완료(빈 결과): session_id=%s", session_id)
    ...
    def _save_final_utterance_and_events(
        ...
        *,
        ...
        if self._should_emit_live_final(final_queue_delay_ms):
        ...
        if not self._should_emit_live_final(final_queue_delay_ms):
            logger.info(
                "live final 전송 생략: session_id=%s segment_id=%s final_queue_delay_ms=%s max_live_delay_ms=%s",
                session_id,
                segment_id,
                final_queue_delay_ms,

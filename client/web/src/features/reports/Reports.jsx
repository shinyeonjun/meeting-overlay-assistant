  ...
  async function handleGenerateReport(sessionId) {
    try {
      ...
      if (completedJob === null) {
        setError("회의록 생성이 지연되고 있습니다. worker 상태를 확인한 뒤 잠시 후 다시 확인해 주세요.");
      }
    } catch (nextError) {
      setError(nextError instanceof Error ? nextError.message : "회의록 생성 요청에 실패했습니다.");
    } finally {
    ...
    <div className="reports-board animate-fade-in">
      ...
      <div className="workspace-grid reports-grid">
        ...
        <section className="workspace-panel">
          ...
          <LinkedStatusList
            ...
            onOpenSession={onOpenSession}
            emptyText="현재 처리 중인 회의록 job이 없습니다."
          />
        </section>
      </div>
      ...
      <section className="workspace-panel">
        ...
        {reports.length > 0 ? (
          ...
          <div className="panel-empty">생성된 회의록이 아직 없습니다.</div>
        )}
      </section>

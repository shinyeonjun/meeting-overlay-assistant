...
class SQLiteReportShareRepository(ReportShareRepository):
    """SQLite 기반 리포트 공유 저장소."""
        ...
        *,
        ...
        with self._database.connect() as connection:
            rows = connection.execute(
                ...
                INNER JOIN users AS shared_by ON shared_by.id = report_shares.shared_by_user_id
                WHERE report_shares.shared_with_user_id = ?
                ORDER BY report_shares.created_at DESC
                LIMIT ?
                """,

...
class RedisNoteCorrectionJobQueue:
    """Redis list를 이용해 노트 보정 job을 깨운다."""
    ...
    def publish(self, job_id: str) -> bool:
        ...
        try:
            ...
            logger.exception(
                "note correction job 큐 발행 실패: queue_key=%s job_id=%s",
                self._queue_key,
                job_id,
            )
            return False
    ...
    def wait_for_job(self, timeout_seconds: float) -> str | None:
        ...
        try:
            ...
            logger.exception(
                "note correction job 큐 대기 실패: queue_key=%s timeout_seconds=%s",
                self._queue_key,
                timeout_seconds,
            )
            return None

...
from server.app.services.audio.pipeline.final.helpers.final_caption_payloads import (
    emit_outgoing_final_caption,
)
from server.app.services.audio.pipeline.final.helpers.runtime_persistence import (
    _list_recent_utterances,
    _persist_or_keep_runtime_event,
    _persist_or_keep_runtime_utterance,
    _resolve_next_utterance_sequence,
)
...
def save_final_utterance_and_events(
    ...
    *,
    ...
    emitted_live_final = emit_outgoing_final_caption(
        service,
        session_id=session_id,
        saved_utterance=saved_utterance,
        segment_id=segment_id,
        outgoing_seq_num=outgoing_seq_num,
...
def _record_final_transcription(
    service,
    *,
    session_id: str,
...
def _persist_live_question_events(
    service,
    *,
    session_id: str,
    saved_utterance: Utterance,
    ...
    for event in service._analyzer_service.analyze(saved_utterance):
        event_candidate = _normalize_live_event_candidate(
            event,
            saved_utterance=saved_utterance,
            input_source=input_source,
        )
        ...
        _upsert_saved_event(saved_events, saved_event)
        logger.debug(
...
def _normalize_live_event_candidate(
    event: MeetingEvent,
    *,
    saved_utterance: Utterance,
    input_source: str | None,
) -> MeetingEvent | None:
    if event.event_type not in _LIVE_EVENT_TYPES:
        return None
...
def _upsert_saved_event(saved_events, saved_event: MeetingEvent) -> None:
    existing_index = next(
        (
            index

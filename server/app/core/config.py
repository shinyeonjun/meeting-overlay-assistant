...
class AppConfig:
    ...
    def from_env(cls) -> "AppConfig":
        return cls(
            ...
            persistence_backend=_get_env("PERSISTENCE_BACKEND", "sqlite") or "sqlite",
            database_path=_get_path("DATABASE_PATH", "server/data/meeting_overlay.db"),
            postgresql_dsn=_get_env("POSTGRESQL_DSN"),
            log_file_path=_get_path("LOG_FILE_PATH", "server/data/logs/caps-server.log"),
            ...
            auth_session_ttl_hours=_get_int("AUTH_SESSION_TTL_HOURS", 12),
            analysis_rules_config_path=_get_path("ANALYSIS_RULES_CONFIG_PATH", "server/config/analysis_rules.json"),
            ...
            retrieval_embedding_backend=_get_env("RETRIEVAL_EMBEDDING_BACKEND", "ollama") or "ollama",
            retrieval_embedding_model=_get_env(
                "RETRIEVAL_EMBEDDING_MODEL",
                "nomic-embed-text:latest",
            )
            or "nomic-embed-text:latest",
            retrieval_embedding_base_url=_get_env(
                "RETRIEVAL_EMBEDDING_BASE_URL",
                "http://127.0.0.1:11434",
            ),
            retrieval_embedding_timeout_seconds=_get_int(
                "RETRIEVAL_EMBEDDING_TIMEOUT_SECONDS",
                20,
            ),
            retrieval_embedding_dimensions=_get_int("RETRIEVAL_EMBEDDING_DIMENSIONS", 768),
            retrieval_chunk_target_chars=_get_int("RETRIEVAL_CHUNK_TARGET_CHARS", 1000),
            retrieval_chunk_overlap_chars=_get_int("RETRIEVAL_CHUNK_OVERLAP_CHARS", 160),
            retrieval_search_candidate_limit=_get_int(
                "RETRIEVAL_SEARCH_CANDIDATE_LIMIT",
                100,
            ),
            ...
            live_stream_worker_count=_get_int("LIVE_STREAM_WORKER_COUNT", 1),
            live_stream_pending_chunks_per_stream=_get_int(
                "LIVE_STREAM_PENDING_CHUNKS_PER_STREAM",
                3,
            ),
            live_stream_max_running_sessions=_get_int("LIVE_STREAM_MAX_RUNNING_SESSIONS", 4),
        )

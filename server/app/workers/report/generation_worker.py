...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--queue-block-seconds",
        type=float,
        default=max(settings.report_job_queue_block_seconds, 1),
        help="Redis 큐에서 job 신호를 기다리는 최대 시간입니다.",
    )
    parser.add_argument(
        "--poll-interval-seconds",
        type=float,
        default=max(settings.report_job_fallback_poll_seconds, 1),
    ...
    *,
    ...
    for claimed_job in claimed_jobs:
        heartbeat = JobLeaseHeartbeat(
            ...
            renew_lease=lambda job_id=claimed_job.id: report_job_service.renew_job_lease(
                job_id=job_id,
                worker_id=worker_id,
                lease_duration_seconds=lease_seconds,
            ),
        ...
        with heartbeat.running():
            processed_jobs.append(
                report_job_service.process_job(claimed_job.id, expected_worker_id=worker_id)
            )
...
def main() -> int:
    ...
    report_job_service = get_report_generation_job_service()
    queue_enabled = report_job_service.has_queue
    ...
    try:
        last_sweep_at = 0.0
        while True:
            dispatched_job_id = report_job_service.wait_for_dispatched_job(queue_block_seconds)
            should_sweep = False
